...
--------------------------------------------------------------------------------
-- 3) Run shell commands synchronously using :systemlist (blocking)
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- 6) Run dbsqlcli and process output with Nushell in a vertical terminal split
--------------------------------------------------------------------------------
local function run_dbsqlcli_with_nushell(sql_file)
  -- 1) Create a new vertical split on the right with width=120 columns
  vim.cmd 'botright 120vnew'
  vim.cmd 'startinsert'

  -- 2) Prepare the command to execute the SQL query and process with Nushell
  local dbsqlcli_cmd = string.format('dbsqlcli -e %s | from csv | explore', vim.fn.shellescape(sql_file))
  local nu_cmd = string.format('nu -c %s', vim.fn.shellescape(dbsqlcli_cmd))

  -- 3) Turn the current buffer into a terminal that runs `nu_cmd`
  vim.fn.termopen(nu_cmd, {
    on_exit = function(_, code, _)
      if code == 0 then
        print '[dbt_preview] SQL query executed and rendered successfully.'
      else
        print('[dbt_preview] SQL query execution failed with code: ' .. code)
      end
    end,
  })
end
...
function M.preview_compiled_sql()
  ...
  print '[dbt_preview] Executing SQL query and rendering output with Nushell...'

  -- 3) Execute SQL query and render output using Nushell
  run_dbsqlcli_with_nushell(temp_sql_file)
end
